...
function createApp(name, verbose, version, useNpm, template) {
  const root = path.resolve(name); // 获取当前目录
...
/**
 * 获取react-scripts和拼接版本号
 * @param {*} version 
 * @param {*} originalDirectory 
 */
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = 'react-scripts';
  const validSemver = semver.valid(version); // 规范化version
  if (validSemver) {
...
// Extract package name from tarball url or path.【未读】
...
/**
 * 检验项目名是否合法（能不能作为一个文件夹的名字）
 * @param {*} appName 
 */
function checkAppName(appName) {
  /**
   * 来自validate-npm-package-name包
  返回
  {
...
function isSafeToCreateProjectIn(root, name) {
  ...
  const conflicts = fs
    .readdirSync(root) // return An array of filenames
  ...
  currentFiles.forEach(file => {
    errorLogFilePatterns.forEach(errorLogFilePattern => {
      ...
      if (file.indexOf(errorLogFilePattern) === 0) {
        fs.removeSync(path.join(root, file)); // 来自node-fs-extra: Removes a file or directory
      }
    });
  });
  return true;
